...
export default function TabComponent({ backgroundColor, serviceId }) {
  ...
  useEffect(() => {
    const fetchManagedCampus = async () => {
      try {
        ...
        // Filter managed campus data based on the service ID
  ...
  useEffect(() => {
    const fetchExecutionOverviews = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5353/getAll/execution_overviews/${id}`
        );
        setExecutionOverviews(response.data); // Assuming response is an array of executionOverviews
      } catch (error) {
  ...
  // Populate uniqueYears from executionOverviews
